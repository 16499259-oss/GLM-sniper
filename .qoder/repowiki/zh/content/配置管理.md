...
## 简介
本文件系统性梳理 GLM Sniper 的配置管理体系，涵盖全局配置常量（API 端点、产品配置、默认参数）、TypeScript/构建与样式配置、以及运行时配置的加载与使用方式。文档同时提供配置项说明、修改指南、环境变量使用与安全注意事项，以及配置优先级与覆盖规则。
...
- 套餐配置与默认产品
  ...
  - 示例路径：[src/lib/config.ts:28-73](file://src/lib/config.ts#L28-L73)

- 库存检查与产品ID映射
  - STOCK_CHECK_IDS：库存状态与各套餐配置的标识ID集合。
  - 示例路径：[src/lib/config.ts:76-81](file://src/lib/config.ts#L76-L81)

- API 端点配置
  - API_ENDPOINTS：聚合所有业务相关端点（产品、库存、支付、用户等），统一前缀与路径。
  - 示例路径：[src/lib/config.ts:83-101](file://src/lib/config.ts#L83-L101)
...
- 类型与接口
  ...
  - 示例路径：[src/lib/config.ts:6-26](file://src/lib/config.ts#L6-L26)
...
```mermaid
sequenceDiagram
participant UI as "前端组件<br/>PlanSelector/ModeSwitcher"
...
UI->>Hook : 选择套餐/模式
Hook->>CFG : 读取 PLANS/PRODUCT_IDS/API_ENDPOINTS
alt API 模式
...
## 结论
本项目的配置管理以集中式常量为核心，结合 Vite/TypeScript/Tailwind 的现代工具链，形成清晰、可维护、可扩展的配置体系。通过明确的类型约束与统一的端点管理，降低了跨模块耦合风险；通过代理与自动化脚本，兼顾了易用性与性能。建议在生产环境中进一步完善环境变量注入与密钥管理策略。
...
- 套餐与产品ID
  ...
  - 参考路径：[src/lib/config.ts:28-73](file://src/lib/config.ts#L28-L73)

- API 端点
  - 修改端点：更新 API_ENDPOINTS 对应键值。
  - 参考路径：[src/lib/config.ts:83-101](file://src/lib/config.ts#L83-L101)
