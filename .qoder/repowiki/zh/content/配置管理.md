...
## 更新摘要
**变更内容**
- 支付系统标准化完成：WeChat Pay渠道代码从'WECHAT_WEB'更新为'WE_CHAT'，Alipay渠道代码从'ALIPAY_WEB'更新为'ALI'
- 新增支付方法配置系统，包括 PaymentMethod 类型定义、PAYMENT_METHODS 常量和 DEFAULT_PAYMENT_METHOD 设置
- 更新套餐配置系统，新增支付周期计算和价格格式化功能
- 新增 PaymentMethodSelector 组件用于支付方式选择
- 扩展 useSniper 钩子以支持支付方法状态管理
- **更新**：PRODUCT_IDS中的产品ID从占位符更新为真实的智谱AI产品代码，包括Lite、Pro、Max套餐的月付、季付、年付产品ID均已更新为实际的product-xxxxxx格式
...
## 简介
本文件系统性梳理 GLM Sniper 的配置管理体系，涵盖全局配置常量（API 端点、产品配置、默认参数、支付方法配置）、TypeScript/构建与样式配置、以及运行时配置的加载与使用方式。文档同时提供配置项说明、修改指南、环境变量使用与安全注意事项，以及配置优先级与覆盖规则。
...
**图表来源**
- [src/lib/config.ts:1-150](file://src/lib/config.ts#L1-L150)
- [src/components/PaymentMethodSelector.tsx:1-55](file://src/components/PaymentMethodSelector.tsx#L1-L55)
- [src/hooks/useSniper.ts:1-483](file://src/hooks/useSniper.ts#L1-L483)
...
- 支付方式类型定义
  ...
  - 支付方式配置：PAYMENT_METHODS 常量包含每种支付方式的显示名称、代码和图标
  - **更新**：WeChat Pay渠道代码标准化为'WE_CHAT'，Alipay渠道代码标准化为'ALI'
  - 默认支付方式：DEFAULT_PAYMENT_METHOD 设置为支付宝

- 套餐配置与默认产品
  - PLANS：定义各套餐的基础信息（名称、价格、产品ID），并可选徽标展示
  - PRODUCT_IDS：按套餐类型与支付周期（月付/季付/年付）映射具体产品ID，**更新**为真实的智谱AI产品代码
  - getDefaultProductId：提供默认产品ID（季付）的便捷查询
  - 示例路径：[src/lib/config.ts:58-120](file://src/lib/config.ts#L58-L120)

- 库存检查与产品ID映射
  - STOCK_CHECK_IDS：库存状态与各套餐配置的标识ID集合
  - 示例路径：[src/lib/config.ts:125-130](file://src/lib/config.ts#L125-L130)

- API 端点配置
  - API_ENDPOINTS：聚合所有业务相关端点（产品、库存、支付、用户等），统一前缀与路径
  - 示例路径：[src/lib/config.ts:132-150](file://src/lib/config.ts#L132-L150)
...
Hook->>CFG : 读取 PLANS/PAYMENT_METHODS/API_ENDPOINTS
alt API 模式
...
- [src/lib/config.ts:58-150](file://src/lib/config.ts#L58-L150)
...
## 结论
本项目的配置管理以集中式常量为核心，结合 Vite/TypeScript/Tailwind 的现代工具链，形成清晰、可维护、可扩展的配置体系。通过明确的类型约束与统一的端点管理，降低了跨模块耦合风险；通过代理与自动化脚本，兼顾了易用性与性能。新增的支付方法配置系统进一步增强了用户体验，提供了灵活的支付方式选择。**更新**：支付系统已完成标准化，确保了与后端接口的兼容性和一致性。**最新更新**：PRODUCT_IDS中的产品ID已更新为真实的智谱AI产品代码，包括Lite、Pro、Max套餐的月付、季付、年付产品ID均已更新为实际的product-xxxxxx格式，为后续的支付流程提供了准确的产品标识。建议在生产环境中进一步完善环境变量注入与密钥管理策略。
...
- 套餐与产品ID
  ...
  - **更新**：PRODUCT_IDS中的产品ID已更新为真实的智谱AI产品代码，包括Lite、Pro、Max套餐的月付、季付、年付产品ID均已更新为实际的product-xxxxxx格式。
  - 参考路径：[src/lib/config.ts:58-120](file://src/lib/config.ts#L58-L120)

- API 端点
  - 修改端点：更新 API_ENDPOINTS 对应键值。
  - 参考路径：[src/lib/config.ts:132-150](file://src/lib/config.ts#L132-L150)

- 默认参数
  - 目标时间格式：YYYY-MM-DD 与 HH:mm。
  - 重试策略：最大重试次数与间隔（在 useSniper 中体现）。
  - 参考路径：[src/hooks/useSniper.ts:115-177](file://src/hooks/useSniper.ts#L115-L177)
