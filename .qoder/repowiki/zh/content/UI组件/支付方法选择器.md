...
## 更新摘要
**变更内容**
- 支付渠道代码从数字索引('1','2','0')更新为描述性字符串('ALIPAY','WECHAT','BALANCE')
- API请求参数简化：移除paymentType字段，改用channelCode直接传递支付方式代码
- 支付方式配置结构保持不变，但代码值已标准化为描述性字符串
...
### 支付方式配置
组件支持三种支付方式：
- **支付宝 (Alipay)**：使用描述性代码 'ALIPAY' 作为渠道代码
- **微信支付 (WeChat)**：使用描述性代码 'WECHAT' 作为渠道代码  
- **账户余额 (Balance)**：使用描述性代码 'BALANCE' 作为渠道代码

**更新** 支付渠道代码已标准化为描述性字符串格式，移除了数字索引的复杂性，使用更直观、易读的字符串标识。
...
Config->>Server : 发送支付请求时包含描述性渠道代码
Note over User,Server : 描述性渠道代码影响后续的支付流程
```
...
每个支付方式都有对应的配置信息：
- **名称**：显示给用户的中文名称
- **代码**：后端识别的支付渠道代码（已更新为描述性字符串格式）
- **图标**：Unicode 字符表示的图标

**更新** 支付渠道代码已标准化为描述性字符串格式：
- 支付宝：'ALIPAY'（原 '1'）
- 微信支付：'WECHAT'（原 '2'）
- 账户余额：'BALANCE'（原 '0'）
...
#### 支付流程集成
支付方式选择直接影响后续的支付流程：
1. **API 模式**：直接使用选择的支付方式调用后端接口，包含标准化的描述性渠道代码
2. **浏览器模式**：通过 Playwright 自动化选择相应的支付选项

**更新** 在 API 模式中，支付渠道代码现在使用标准化的描述性字符串格式：
- 支付宝：PAYMENT_METHODS[paymentMethod].code 返回 'ALIPAY'
- 微信支付：PAYMENT_METHODS[paymentMethod].code 返回 'WECHAT'
- 账户余额：PAYMENT_METHODS[paymentMethod].code 返回 'BALANCE'
...
**更新** 支付系统已完成重要的标准化变更，将支付渠道代码从数字索引格式更新为描述性字符串格式，提高了系统的可维护性和一致性。新的标准化代码格式为：
- 支付宝：'ALIPAY'
- 微信支付：'WECHAT'  
- 账户余额：'BALANCE'
...
**更新** API 请求参数也进行了简化，移除了 paymentType 字段，直接使用 channelCode 作为支付方式标识，使请求更加简洁明了。