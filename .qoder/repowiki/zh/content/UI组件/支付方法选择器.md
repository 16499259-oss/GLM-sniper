...
## 更新摘要
**变更内容**
- 更新支付渠道代码标准化：支付宝使用 'ALI'，微信支付使用 'WE_CHAT'
- 更新UI显示和内部配置映射以匹配新的标准化代码
- 修正支付方式配置中的渠道代码格式
...
### 支付方式配置
组件支持三种支付方式：
- **支付宝 (Alipay)**：使用 ALI 渠道代码
- **微信支付 (WeChat)**：使用 WE_CHAT 渠道代码  
- **账户余额 (Balance)**：使用 BALANCE 渠道代码

**更新** 支付渠道代码已标准化，移除了冗余的 WEB 后缀，使用更简洁的标准代码格式。
...
Config->>Server : 发送支付请求时包含支付方式
Note over User,Server : 支付方式影响后续的支付流程
```

**图表来源**
- [src/components/PaymentMethodSelector.tsx:24-27](file://src/components/PaymentMethodSelector.tsx#L24-L27)
- [src/hooks/useSniper.ts:457-458](file://src/hooks/useSniper.ts#L457-L458)
- [src/lib/config.ts:85-90](file://src/lib/config.ts#L85-L90)
...
**更新** 支付渠道代码已标准化：
- 支付宝：'ALI'（原 'ALIPAY_WEB'）
- 微信支付：'WE_CHAT'（原 'WECHAT_WEB'）
- 账户余额：'BALANCE'
...
**图表来源**
- [src/hooks/useSniper.ts:457-458](file://src/hooks/useSniper.ts#L457-L458)
- [src/hooks/useSniper.ts:462](file://src/hooks/useSniper.ts#L462)

#### 支付流程集成
支付方式选择直接影响后续的支付流程：
1. **API 模式**：直接使用选择的支付方式调用后端接口，包含标准化的渠道代码
2. **浏览器模式**：通过 Playwright 自动化选择相应的支付选项

**更新** 在 API 模式中，支付渠道代码现在使用标准化格式：
- 支付宝：PAYMENT_METHODS[paymentMethod].code 返回 'ALI'
- 微信支付：PAYMENT_METHODS[paymentMethod].code 返回 'WE_CHAT'
...
**更新** 支付渠道代码已标准化，移除了冗余的 WEB 后缀，使用更简洁的标准格式。
...
**更新** 支付系统已完成标准化变更，使用更简洁、统一的渠道代码格式，提高了系统的可维护性和一致性。新的标准化代码格式为：
- 支付宝：'ALI'
- 微信支付：'WE_CHAT'  
- 账户余额：'BALANCE'

支付方法选择器虽然功能相对简单，但在整个抢购流程中发挥着重要作用，为用户提供了清晰、直观的支付方式选择体验。